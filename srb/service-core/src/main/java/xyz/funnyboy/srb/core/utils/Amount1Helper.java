package xyz.funnyboy.srb.core.utils;
...
/**
 * 等额本息工具类 校验网址：http://www.xjumc.com/ 等额本息是指一种贷款的还款方式，是在还款期内，每月偿还同等数额的贷款(包括本金和利息)，和等额本金是不一样的概念，虽然刚开始还款时每月还款额可能会低于等额本金还款方式，但是最终所还利息会高于等额本金还款方式，该方式经常被银行使用。
 * <p>
 * 每月还款数额计算公式如下： 每月还款额=贷款本金×[月利率×(1+月利率) ^ 还款月数]÷{[(1+月利率) ^ 还款月数]-1}
 *
 * @author VectorX
 * @version 1.0.0
 * @date 2024/01/26
    ...
    public static Map<Integer, BigDecimal> getPerMonthInterest(BigDecimal invest, BigDecimal yearRate, int totalMonth) {
        ...
        for (int i = 1; i < totalMonth + 1; i++) {
            ...
            BigDecimal sub = BigDecimal
                    .valueOf(Math.pow(1 + monthRate, totalMonth))
                    .subtract(BigDecimal.valueOf(Math.pow(1 + monthRate, i - 1)));
            monthInterest = multiply
                    .multiply(sub)
                    .divide(BigDecimal.valueOf(Math.pow(1 + monthRate, totalMonth) - 1), 8, BigDecimal.ROUND_DOWN);
