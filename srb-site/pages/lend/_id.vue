...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      if (userInfo) {
        this.$axios
            .$get('/api/core/userAccount/auth/getAccountAmount')
            .then(response => {
              this.account = response.data.accountAmount
            })
      }
    },
    ...
    commitInvest() {
      ...
      if (userInfoObj.userType === 2) {
        this.$message.error('借款人不能投资')
        return
      }

      // 判断标的是否超卖：标的已投金额 + 本次投资金额 > 标的总金额
      if (this.lend.investAmount + Number(this.invest.investAmount) > this.lend.amount) {
        this.$message.error('标的已超卖')
        return
      }

      // 是否是lowestAmount的整数倍
      let lowestAmount = this.lend.lowestAmount;
      if (Number(this.invest.investAmount) === 0 || this.invest.investAmount % lowestAmount !== 0) {
        this.$message.error(`投资金额必须是${lowestAmount}的整数倍`)
        return
      }
